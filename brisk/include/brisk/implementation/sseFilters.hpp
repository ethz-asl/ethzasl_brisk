		...
		for(unsigned int j=0; j<maxJ; ){
			...
			for(unsigned int x=0;x<X;++x){
				...
				for(unsigned int y=0;y<Y;++y){
					...
					__m128i i0 = _mm_loadu_si128 ((__m128i*)&src.at<short>(i+y,j+x));
...
__inline__ void filter2D8U(cv::Mat& src, cv::Mat& dst, cv::Mat& kernel){
	...
	for(unsigned int i=0; i<maxI; ++i){
		...
		for(unsigned int j=0; j<maxJ; ){
			...
			_mm_storeu_si128 ((__m128i*)p_hi,_mm_unpackhi_epi16 (result_hi, result_lo));
			_mm_storeu_si128 ((__m128i*)p_lo,_mm_unpacklo_epi16 (result_hi, result_lo));
...
__inline__ void filter2D(cv::Mat& src, cv::Mat& dst, cv::Mat& kernel){
	if(src.type()==CV_8U) filter2D8U<X,Y>(src, dst, kernel);
	else if(src.type()==CV_16S) filter2D16S<X,Y>(src, dst, kernel);
	else assert(0&&"only CV_8U and CV_16S are supported src matrix types");
}
