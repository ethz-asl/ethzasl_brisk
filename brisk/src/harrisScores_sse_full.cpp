...
inline void smoothGauss_epi16(const short* const data, int i, int j, int cols,
  ...
  // Load.
  const __m128i d_m1_m1_lo = _mm_set_epi32(*(data + (i - 1) * cols + j + 5),
                                           *(data + (i - 1) * cols + j + 3),
                                           *(data + (i - 1) * cols + j + 1),
                                           *(data + (i - 1) * cols+j-1));
  const __m128i d_m1_0_lo = _mm_set_epi32(*(data + (i - 1) * cols + j + 6),
                                          *(data + (i - 1) * cols + j + 4),
                                          *(data + (i - 1) * cols + j + 2),
                                          *(data + (i - 1) * cols+j));
  const __m128i d_m1_p1_lo = _mm_set_epi32(*(data + (i - 1) * cols + j + 7),
                                           *(data + (i - 1) * cols + j + 5),
                                           *(data + (i - 1) * cols + j + 3),
                                           *(data + (i - 1) * cols + j + 1));
  const __m128i d_0_m1_lo = _mm_set_epi32(*(data + (i) * cols + j + 5),
                                          *(data + (i) * cols + j + 3),
                                          *(data + (i) * cols + j + 1),
                                          *(data + (i) * cols+j-1));
  const __m128i d_0_0_lo = _mm_set_epi32(*(data + (i) * cols + j + 6),
                                         *(data + (i) * cols + j + 4),
                                         *(data + (i) * cols + j + 2),
                                         *(data + (i) * cols+j));
  const __m128i d_0_p1_lo = _mm_set_epi32(*(data + (i) * cols + j + 7),
                                          *(data + (i) * cols + j + 5),
                                          *(data + (i) * cols + j + 3),
                                          *(data + (i) * cols + j + 1));
  const __m128i d_p1_m1_lo = _mm_set_epi32(*(data + (i+1) * cols + j + 5),
                                           *(data + (i+1) * cols + j + 3),
                                           *(data + (i+1) * cols + j + 1),
                                           *(data + (i+1) * cols+j-1));
  const __m128i d_p1_0_lo = _mm_set_epi32(*(data + (i+1) * cols + j + 6),
                                          *(data + (i+1) * cols + j + 4),
                                          *(data + (i+1) * cols + j + 2),
                                          *(data + (i+1) * cols+j));
  const __m128i d_p1_p1_lo = _mm_set_epi32(*(data + (i+1) * cols + j + 7),
                                           *(data + (i+1) * cols + j + 5),
                                           *(data + (i+1) * cols + j + 3),
                                           *(data + (i+1) * cols + j + 1));

  const __m128i d_m1_m1_hi=_mm_set_epi32(*(data + (i - 1) * cols + j + 5+1),
                                         *(data + (i - 1) * cols + j + 3+1),
                                         *(data + (i - 1) * cols + j + 1+1),
                                         *(data + (i - 1) * cols+j-1+1));
  const __m128i d_m1_0_hi=_mm_set_epi32(*(data + (i - 1) * cols + j + 6+1),
                                        *(data + (i - 1) * cols + j + 4+1),
                                        *(data + (i - 1) * cols + j + 2+1),
                                        *(data + (i - 1) * cols + j + 1));
  const __m128i d_m1_p1_hi=_mm_set_epi32(*(data + (i - 1) * cols + j + 7+1),
                                         *(data + (i - 1) * cols + j + 5+1),
                                         *(data + (i - 1) * cols + j + 3+1),
                                         *(data + (i - 1) * cols + j + 1+1));
  const __m128i d_0_m1_hi=_mm_set_epi32(*(data + (i) * cols + j + 5+1),
                                        *(data + (i) * cols + j + 3+1),
                                        *(data + (i) * cols + j + 1+1),
                                        *(data + (i) * cols+j-1+1));
  const __m128i d_0_0_hi=_mm_set_epi32(*(data + (i) * cols + j + 6+1),
                                       *(data + (i) * cols + j + 4+1),
                                       *(data + (i) * cols + j + 2+1),
                                       *(data + (i) * cols + j + 1));
  const __m128i d_0_p1_hi=_mm_set_epi32(*(data + (i) * cols + j + 7+1),
                                        *(data + (i) * cols + j + 5+1),
                                        *(data + (i) * cols + j + 3+1),
                                        *(data + (i) * cols + j + 1+1));
  const __m128i d_p1_m1_hi=_mm_set_epi32(*(data + (i+1) * cols + j + 5+1),
                                         *(data + (i+1) * cols + j + 3+1),
                                         *(data + (i+1) * cols + j + 1+1),
                                         *(data + (i+1) * cols+j-1+1));
  const __m128i d_p1_0_hi=_mm_set_epi32(*(data + (i+1) * cols + j + 6+1),
                                        *(data + (i+1) * cols + j + 4+1),
                                        *(data + (i+1) * cols + j + 2+1),
                                        *(data + (i+1) * cols + j + 1));
  const __m128i d_p1_p1_hi=_mm_set_epi32(*(data + (i+1) * cols + j + 7+1),
                                         *(data + (i+1) * cols + j + 5+1),
                                         *(data + (i+1) * cols + j + 3+1),
                                         *(data + (i+1) * cols + j + 1+1));

  // Gaussian smoothing.
  const __m128i s1_lo = _mm_add_epi32(_mm_add_epi32(d_m1_m1_lo, d_m1_p1_lo),
                                      _mm_add_epi32(d_p1_m1_lo, d_p1_p1_lo));
  const __m128i s1_hi=_mm_add_epi32(_mm_add_epi32(d_m1_m1_hi, d_m1_p1_hi),
                                    _mm_add_epi32(d_p1_m1_hi, d_p1_p1_hi));
  const __m128i s2_lo = _mm_add_epi32(_mm_add_epi32(d_0_m1_lo, d_m1_0_lo),
                                      _mm_add_epi32(d_p1_0_lo, d_0_p1_lo));
  const __m128i s2_hi=_mm_add_epi32(_mm_add_epi32(d_0_m1_hi, d_m1_0_hi),
                                    _mm_add_epi32(d_p1_0_hi, d_0_p1_hi));

  result_lo = _mm_add_epi32(_mm_add_epi32(_mm_slli_epi32(d_0_0_lo, 2),
                                          _mm_slli_epi32(s2_lo,1)), s1_lo);
  result_lo = _mm_srai_epi32(result_lo, 4);
  result_hi = _mm_add_epi32(_mm_add_epi32(_mm_slli_epi32(d_0_0_hi, 2),
                                          _mm_slli_epi32(s2_hi,1)), s1_hi);
