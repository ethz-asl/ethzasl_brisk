...
// masks
__m128i mask_lo_epi32 = _mm_set_epi16(0x0000,0xFFFF,0x0000,0xFFFF,0x0000,0xFFFF,0x0000,0xFFFF);
__m128i mask_hi_epi32 = _mm_set_epi16(0xFFFF,0x0000,0xFFFF,0x0000,0xFFFF,0x0000,0xFFFF,0x0000);

// helper function for smoothing
inline void smoothGauss_epi16(const short* const data, int i, int j, int cols, __m128i& result_lo, __m128i& result_hi){
	// load
...
void harrisScores_sse_full(const cv::Mat& src, cv::Mat& scores){
	...
	// masks
	__m128i mask_lo = _mm_set_epi8(0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF);
	__m128i mask_hi = _mm_set_epi8(0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00,0xFF,0x00);
	...
	for(int i=1; i<rows-1;++i){
		...
		for(int j=1; j<cols-1; ){
			// load
			const __m128i src_m1_m1=_mm_loadu_si128((__m128i*)(data+(i-1)*stride+j-1));
			const __m128i src_m1_0=_mm_loadu_si128((__m128i*)(data+(i-1)*stride+j));
			const __m128i src_m1_p1=_mm_loadu_si128((__m128i*)(data+(i-1)*stride+j+1));
			const __m128i src_0_m1=_mm_loadu_si128((__m128i*)(data+(i)*stride+j-1));
			const __m128i src_0_0=_mm_loadu_si128((__m128i*)(data+(i)*stride+j));
			const __m128i src_0_p1=_mm_loadu_si128((__m128i*)(data+(i)*stride+j+1));
			const __m128i src_p1_m1=_mm_loadu_si128((__m128i*)(data+(i+1)*stride+j-1));
			const __m128i src_p1_0=_mm_loadu_si128((__m128i*)(data+(i+1)*stride+j));
			const __m128i src_p1_p1=_mm_loadu_si128((__m128i*)(data+(i+1)*stride+j+1));

			// Scharr x
			const __m128i dx_lo = _mm_slli_epi16(_mm_add_epi16 (_mm_add_epi16(_mm_mullo_epi16(const_10_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_0_m1),_mm_and_si128(mask_lo,src_0_p1)))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_m1_m1),_mm_and_si128(mask_lo,src_m1_p1))))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_p1_m1),_mm_and_si128(mask_lo,src_p1_p1)))),3);
			const __m128i dx_hi = _mm_slli_epi16(_mm_add_epi16 (_mm_add_epi16(_mm_mullo_epi16(const_10_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_0_m1),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_0_p1),1)))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_m1_m1),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_m1_p1),1))))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_p1_m1),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_p1_p1),1)))),3);

			// Scharr y
			const __m128i dy_lo = _mm_slli_epi16(_mm_add_epi16 (_mm_add_epi16(_mm_mullo_epi16(const_10_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_m1_0),_mm_and_si128(mask_lo,src_p1_0)))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_m1_m1),_mm_and_si128(mask_lo,src_p1_m1))))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_and_si128(mask_lo,src_m1_p1),_mm_and_si128(mask_lo,src_p1_p1)))),3);
			const __m128i dy_hi = _mm_slli_epi16(_mm_add_epi16 (_mm_add_epi16(_mm_mullo_epi16(const_10_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_m1_0),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_p1_0),1)))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_m1_m1),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_p1_m1),1))))
					,_mm_mullo_epi16(const_3_epi16,_mm_sub_epi16(_mm_srli_si128(_mm_and_si128(mask_hi,src_m1_p1),1),_mm_srli_si128(_mm_and_si128(mask_hi,src_p1_p1),1)))),3);
			...
			_mm_storeu_si128((__m128i*)(DxDx1+i*cols+j),_mm_unpacklo_epi16(i_lo_dx_dx,i_hi_dx_dx));
			_mm_storeu_si128((__m128i*)(DxDx1+i*cols+j+8),_mm_unpackhi_epi16(i_lo_dx_dx,i_hi_dx_dx));
			_mm_storeu_si128((__m128i*)(DxDy1+i*cols+j),_mm_unpacklo_epi16(i_lo_dx_dy,i_hi_dx_dy));
			_mm_storeu_si128((__m128i*)(DxDy1+i*cols+j+8),_mm_unpackhi_epi16(i_lo_dx_dy,i_hi_dx_dy));
			_mm_storeu_si128((__m128i*)(DyDy1+i*cols+j),_mm_unpacklo_epi16(i_lo_dy_dy,i_hi_dy_dy));
			_mm_storeu_si128((__m128i*)(DyDy1+i*cols+j+8),_mm_unpackhi_epi16(i_lo_dy_dy,i_hi_dy_dy));
	...
	for(int i=2; i<rows-2;++i){
		...
		for(int j=2; j<cols-2; ){
			...
			__m128i dxdx_p_dydy_div_by_2_lo=_mm_srai_epi32(_mm_add_epi32(dxdx_lo,dydy_lo),1);
			__m128i dxdx_p_dydy_div_by_2_hi=_mm_srai_epi32(_mm_add_epi32(dxdx_hi,dydy_hi),1);

			__m128i score_lo=_mm_sub_epi32(_mm_sub_epi32(
					_mm_mullo_epi32(dxdx_lo,dydy_lo),
							_mm_mullo_epi32(dxdy_lo,dxdy_lo)),
							_mm_srai_epi32(_mm_mullo_epi32(dxdx_p_dydy_div_by_2_lo,dxdx_p_dydy_div_by_2_lo),2));
			__m128i score_hi=_mm_sub_epi32(_mm_sub_epi32(
					_mm_mullo_epi32(dxdx_hi,dydy_hi),
							_mm_mullo_epi32(dxdy_hi,dxdy_hi)),
							_mm_srai_epi32(_mm_mullo_epi32(dxdx_p_dydy_div_by_2_hi,dxdx_p_dydy_div_by_2_hi),2));

			// store *((int*)scores.data+i*stride+j)
			_mm_storeu_si128((__m128i*)(((int*)scores.data+i*stride+j)),_mm_unpacklo_epi32(score_lo,score_hi));
			_mm_storeu_si128((__m128i*)(((int*)scores.data+i*stride+j+4)),_mm_unpackhi_epi32(score_lo,score_hi));
