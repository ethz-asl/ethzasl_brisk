...
void HarrisScoreCalculatorFloat::get2dMaxima(std::vector<PointWithScore>& points, float absoluteThreshold){
...
void HarrisScoreCalculatorFloat::getCovarEntries(
			...
			for(unsigned int x=0;x<X;++x){
				...
				for(unsigned int y=0;y<Y;++y){
					...
					if(jump==1){
						...
						i0 = _mm_setr_ps(float(*p),float(*(p+1)),float(*(p+2)),float(*(p+3)));
					}
					else{
...
void HarrisScoreCalculatorFloat::cornerHarris(
	...
	for(unsigned int i=0; i<maxI; ++i){
		...
		for(unsigned int j=0; j<maxJ; ){
			...
			__m128 trace_sq_00625=_mm_mul_ps(trace_sq,_mm_setr_ps(0.0625,0.0625,0.0625,0.0625));
