...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  try
  {
	  ...
	  matcher->radiusMatch(descriptorsLast,descriptors,
	  						matches,60);

	  // make it reasonable for display
	  double max;
	  double min;
	  cv::minMaxIdx(img_mono32F, &min,&max);
	  img_mono32F=(img_mono32F-min)*(1.0/(max-min));
	  //cv::minMaxIdx(half_img_mono32F, &min,&max);
	  half_img_mono32F=(half_img_mono32F-min)*(1.0/(max-min));
	  cv::Mat img_mono32F_scaled=img_mono32F*256.0;
	  cv::Mat half_img_mono32F_scaled=half_img_mono32F*256.0;
	  cv::Mat img_mono8;
	  cv::Mat half_img_mono8;
	  ...
	  cv::drawKeypoints(img_mono8,keypoints,drawing,
			  cv::Scalar(255,0,0),cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS); // blue,multisize

	  cv::Mat matchImg;
	  cv::drawMatches(img_mono8Last,keypointsLast,img_mono8,keypoints,matches,matchImg,
...
int main(int argc, char **argv)
{
  ...
  detector = new brisk::ScaleSpaceFeatureDetector<brisk::HarrisScoreCalculatorFloat>(2,14,130);
  extractor = new cv::BriskDescriptorExtractor(path+"/brisk.ptn",true,true);
  ...
  image_transport::Subscriber sub = it.subscribe("cam3/image_raw", 1, imageCallback);
  ros::spin();
  cvDestroyWindow("Raw");
