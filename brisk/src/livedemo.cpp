...
public:
	ImageSubscriber(ros::NodeHandle &n,
			...
			cv::Ptr<cv::DescriptorMatcher> descriptorMatcher, float matchingThreshold) :
				n_(n), it_(n_)
	{
	...
	void imageCallback(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		float dT=float(tNew_.tv_sec-tOld_.tv_sec)+float(tNew_.tv_usec-tOld_.tv_usec)/1000000.0f;
		if(tOld_.tv_sec==0&&tOld_.tv_usec==0) {
			dT=0.1;
		}
		if(dT==0.0f) dT_=dT;
		else dT_=0.9f*dT_+0.1f*dT;
		...
		gettimeofday(&tEnd, 0);
		sharedData.timeleft = dT_-float(tEnd.tv_sec-tNew_.tv_sec)-float(tEnd.tv_usec-tNew_.tv_usec)/1000000.0f;
...
public:
	...
	{
		assert(!thread_);
		thread_ = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&BriskVisualizer::visualize, this)));
	}
	...
	void visualize(){
		...
		while (!stoprequested_&&!ros::isShuttingDown()){
			...
			while (true){
				...
				boost::mutex::scoped_try_lock l(sharedData.mutex);
				if(l.owns_lock()){
					if(tryCapture&&(sharedData.referenceImage.size()<sharedData.numRefImages)){
						...
						//cv::waitKey();
						cv::cvtColor(tmp.back(), sharedData.imgRGBref2[sharedData.referenceImage.size()-1], CV_GRAY2RGB);
			...
			gettimeofday(&end,0);
			sharedData.timeleft-=float(end.tv_sec-start.tv_sec)+float(end.tv_usec-start.tv_usec)/1000000.0f;
...
void help(char** argv){
	std::cout << "This command line tool lets you evaluate different keypoint "
			...
			<< "usage:" << std::endl
			<< argv[0] << " <detector> <descriptor> <matcher> <threshold> <noRefFrame> [videoFname]" << std::endl
			<< "    " << "detector:   Feature detector, e.g. FAST, or BRISK. You can add the "<< std::endl
			<< "    " << "            threshold, e.g. BRISK80 or SURF2000"<< std::endl
			<< "    " << "descriptor: Feature descriptor, e.g. SURF, BRIEF, BRISK or U-BRISK."<< std::endl
			<< "    " << "matcher:    Can be FlannBased, BruteForce, BruteForce-L1,"<< std::endl
			<< "    " << "            BruteForce-Hamming, BruteForce-HammingLUT, "<<std::endl
			<< "    " << "            or BruteForce-HammingSse (fastest for Hamming)."<< std::endl
			<< "    " << "threshold:  Matching threshold."<< std::endl
			<< "    " << "noRefFrame: Number of reference frames, 1 or 2."<< std::endl
			<< "    " << "videoFname: Optional raw video output file." << std::endl;
}
...
int main(int argc, char ** argv) {
	...
	if (detector.empty()){
		std::cout << "Detector " << argv[1] << " not recognized. Check spelling!" << std::endl;
		return 2;
	}
	...
	if (descriptorExtractor.empty()){
		std::cout << "Descriptor " << argv[2] << " not recognized. Check spelling!" << std::endl;
		return 3;
	}
	...
	ros::NodeHandle nh;
	ImageSubscriber imageSubscriber(nh,detector,descriptorExtractor,descriptorMatcher,matchingThreshold);
	...
	while(visualizer.isrunning()&&!ros::isShuttingDown()){
		...
		if(!imagesInitialized){
			// init shared images
			sharedData.imgRGB=cv::Mat::zeros(currentImage->image.rows+currentImage->image.rows/sharedData.numRefImages,currentImage->image.cols,CV_8UC3);

			// display it:
			sharedData.imgRGBmain=sharedData.imgRGB.rowRange(currentImage->image.rows/sharedData.numRefImages,
					currentImage->image.rows+currentImage->image.rows/sharedData.numRefImages);
			// create the other submatrices as well
			for(unsigned int i=0; i<sharedData.numRefImages; i++){
				sharedData.imgRGBref.push_back(sharedData.imgRGB(cv::Range(0,currentImage->image.rows/sharedData.numRefImages),
						cv::Range(i*currentImage->image.cols/sharedData.numRefImages,(i+1)*currentImage->image.cols/sharedData.numRefImages)));
				sharedData.imgRGBref2.push_back(cv::Mat::zeros(currentImage->image.rows/sharedData.numRefImages,currentImage->image.cols/sharedData.numRefImages,CV_8UC3));
			}
		...
		if(videoOut&&!writerIsOpen){
			writerIsOpen=writer.open(videoFname, CV_FOURCC('I', 'Y', 'U', 'V'),25.0,cv::Size(sharedData.imgRGB.cols,sharedData.imgRGB.rows));
		}
		...
		for(unsigned int i=0; i<sharedData.referenceImage.size(); i++){
			...
			descriptorMatcher->radiusMatch(
					sharedData.referenceImage[i].descriptors,currentImage->descriptors,
					currentMatches->at(i)/**pmatch*/,sharedData.matchingThreshold);
		}
		l.unlock();
		...
		if(sharedData.referenceImage.size()==0){
			for(unsigned int i=0; i<ksize; i++){
				cv::circle(sharedData.imgRGBmain,currentImage->keypoints[i].pt,
						currentImage->keypoints[i].size/4.0,cv::Scalar(255,0,0),2);
			}
		}
		else{
			...
			for(unsigned int i=0; i<ksize2; i++){
				cv::circle(sharedData.imgRGBmain,currentImage->keypoints[i].pt,
						currentImage->keypoints[i].size/4.0,cv::Scalar(0,0,255),2);
			}
			...
			for(unsigned int r=0; r<rSize; r++){
				...
				const unsigned int ksize=sharedData.referenceImage[r].keypoints.size();
				for(unsigned int i=0; i<ksize; i++){
					cv::circle(sharedData.imgRGBref[r],
				...
				for(unsigned int m=0; m<msize; m++){
					if((currentMatches->at(r))[m].size()==0) continue;
					...
					cv::circle(sharedData.imgRGBmain,currentImage->keypoints[ind1].pt,
							...
							cv::Scalar(0,255,0),2);
					cv::circle(sharedData.imgRGBref.at(r),sharedData.referenceImage.at(r).keypoints.at(ind2).pt*scaling,
							sharedData.referenceImage.at(r).keypoints.at(ind2).size/4.0*scaling,
		...
		if(sharedData.timeleft<0.0f)
			ROS_WARN("Framerate overrun!");
		else sharedData.hasNewData=true;
	}
	visualizer.stop();
