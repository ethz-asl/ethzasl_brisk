...
void HarrisFeatureDetector::setRadius(double radius){
	...
	for(int x=0; x<2*16-1; ++x){
		for(int y=0; y<2*16-1; ++y){
			_LUT.at<float>(y,x)=
					std::max(1-double((radius/2.0-x)*(radius/2.0-x)+(radius/2.0-y)*(radius/2.0-y))/double(radius/2.0*radius/2.0),0.0);
		}
	}
}

// X and Y denote the size of the mask
__inline__ void HarrisFeatureDetector::getCovarEntries(
...
__inline__ void HarrisFeatureDetector::cornerHarris(
...
inline void HarrisFeatureDetector::nonmaxSuppress(const cv::Mat& scores, std::vector<cv::KeyPoint>& keypoints){
	...
	for( int j = 2; j < rows_end ; ++j ){
		...
		while(p<p_end){
			...
			keypoints.push_back(cv::KeyPoint(cv::Point2f(i,j), 10, -1, *center, 0, -1));
		}
	}
}
...
__inline__ void HarrisFeatureDetector::enforceUniformity(
	...
	// store occupancy
	cv::Mat occupancy;
	occupancy=cv::Mat::zeros((scores.rows)/2+32,(scores.cols)/2+32,CV_8U);
	...
	for(std::vector<cv::KeyPoint>::iterator it=keypoints.begin(); it!=keypoints.end(); ++it){
		...
		for(int y=0; y<2*16-1; ++y){
			...
			__m128i mask1 = _mm_set_epi8(_LUT.at<float>(y,15)*nsc,_LUT.at<float>(y,14)*nsc,_LUT.at<float>(y,13)*nsc,_LUT.at<float>(y,12)*nsc,_LUT.at<float>(y,11)*nsc,_LUT.at<float>(y,10)*nsc,_LUT.at<float>(y,9)*nsc,_LUT.at<float>(y,8)*nsc,_LUT.at<float>(y,7)*nsc,_LUT.at<float>(y,6)*nsc,_LUT.at<float>(y,5)*nsc,_LUT.at<float>(y,4)*nsc,_LUT.at<float>(y,3)*nsc,_LUT.at<float>(y,2)*nsc,_LUT.at<float>(y,1)*nsc,_LUT.at<float>(y,0)*nsc);
			__m128i mask2 = _mm_set_epi8(0,_LUT.at<float>(y,30)*nsc,_LUT.at<float>(y,29)*nsc,_LUT.at<float>(y,28)*nsc,_LUT.at<float>(y,27)*nsc,_LUT.at<float>(y,26)*nsc,_LUT.at<float>(y,25)*nsc,_LUT.at<float>(y,24)*nsc,_LUT.at<float>(y,23)*nsc,_LUT.at<float>(y,22)*nsc,_LUT.at<float>(y,21)*nsc,_LUT.at<float>(y,20)*nsc,_LUT.at<float>(y,19)*nsc,_LUT.at<float>(y,18)*nsc,_LUT.at<float>(y,17)*nsc,_LUT.at<float>(y,16)*nsc);
