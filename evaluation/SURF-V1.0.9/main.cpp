...
int main (int argc, char **argv)
{
  ...
  osmapping::os_TIME tim1, tim2, tim3; //STS
  // verbose output
  bool bVerbose = true;
  ...
  // stop measuring the detection time, we're all done
  ...
  // stop measuring the description time
  osmapping::os_GetTime(&tim3);
  ...
  if( bVerbose ){
    ...
    cout << "Description time: " << osmapping::os_TimeDiff(&tim3, &tim2) << " ms" << endl;
  }
...
void saveIpoints(string sFileName, const vector< Ipoint >& ipts, bool bVerbose, bool bLaplacian) 
{
  ...
  // comparisons.
  for (unsigned n=0; n<ipts.size(); n++){
    // circular regions with diameter 2.5 x scale
    sc = 1.25 * ipts[n].scale; sc*=sc;
