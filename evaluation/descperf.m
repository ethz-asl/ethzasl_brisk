function [cmatch_nn, tmatch_nn,cmatch_sim,tmatch_sim,cmatch_rn,tmatch_rn, corresp_nn, corresp_sim]=descperf(file1,file2,Hom,imf1,imf2,corresp_nn,twi)
%
%
...
tmatch_nn(1)=1;
for i=2:dss-.5*SIZER
tmatch_nn(i)=ceil(tmatch_nn(i-1)+400/(SIZER));
end
for i=dss-.5*SIZER:dss-.2*SIZER
tmatch_nn(i)=ceil(tmatch_nn(i-1)+1000/SIZER);
end
for i=dss-.2*SIZER:dss
tmatch_nn(i)=ceil(tmatch_nn(i-1)+3000/SIZER);
end
%tmatch_nn'
...
feat(c1,6)=d1;
feat(c1,7)=d2;
scales(c1)=sqrt(feat(c1,6)*feat(c1,7)); % lestefan: what the heck is this? and why is it not used??
